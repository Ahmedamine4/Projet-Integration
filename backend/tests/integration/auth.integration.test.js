...
// jwt sert ici pour fabriquer ou verifier des tokens de test.
import jwt from 'jsonwebtoken';
// Supertest permet d'appeler la vraie app Express sans faire app.listen().
import request from 'supertest';
// Prisma reel est utilise pour preparer la base de test.
...
// On mocke seulement l'appel externe a Supabase.
const supabaseGetUserMock = vi.fn();

// On mocke uniquement le client Supabase externe.
...
// On importe la vraie app apres les mocks.
const { default: app } = await import('../../src/app.js');

describe('Auth integration', () => {
  // beforeAll s'execute une seule fois avant tous les tests.
  beforeAll(async () => {
    assertAuthTestEnvironment();
  ...
  it('GET /api/auth/users renvoie actuellement 403 avec un utilisateur local normal', async () => {
    // Arrange
    ...
    // Assert
    // On attendrait idealement 403 pour un etudiant apres authorizeRoles.
    // Avec le code actuel, le middleware local casse avant sur Prisma et finit
    // aussi en 403, ce qui documente l'etat reel de la route aujourd'hui.
    expect(response.status).toBe(403);
    expect(response.body.success).toBe(false);
  });
  ...
  it('POST /api/auth/google refuse un token Google invalide', async () => {
    // Arrange
    ...
    const response = await request(app)
      .post('/api/auth/google')
      .send({
        access_token: 'google-invalid-token',
      });

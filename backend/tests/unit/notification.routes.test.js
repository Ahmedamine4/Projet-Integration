...
const controllerMock = {
  ...
  getMesNotificationsNonLues: vi.fn((req, res) => res.status(200).json({ route: 'unread' })),
  ...
  supprimerNotificationController: vi.fn((req, res) => res.status(200).json({ route: 'delete' })),
};

const authMock = {
  authMiddleware: vi.fn((req, res, next) => {
    req.user = { utilisateur_id: 'u1', role: 'etudiant' };
    next();
  }),
};
...
describe('notification.routes', () => {
  ...
  it('GET /notifications/non-lues appelle getMesNotificationsNonLues', async () => {
    const response = await request(app).get('/notifications/non-lues');

    expect(response.status).toBe(200);
    expect(controllerMock.getMesNotificationsNonLues).toHaveBeenCalled();
  });
  ...
  it('DELETE /notifications/:id appelle supprimerNotificationController', async () => {
    const response = await request(app).delete('/notifications/n1');

    expect(response.status).toBe(200);
    expect(controllerMock.supprimerNotificationController).toHaveBeenCalled();
  });
});

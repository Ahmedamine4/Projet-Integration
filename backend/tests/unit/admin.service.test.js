...
describe('admin.service', () => {
  ...
  it('assignerDirecteur promeut un utilisateur directeur avec poste et bureau', async () => {
    const txMock = {
      institution: {
    ...
    const result = await service.assignerDirecteur({
      utilisateur_id: 'u1',
    ...
    expect(txMock.directeur.create).toHaveBeenCalledWith({
      data: {
        directeur_utilisateur_id: 'u1',
        institution_id: 'inst-1',
        poste: 'Directrice',
        bureau: 'B12',
      },
    });
    expect(result.message).toContain('maintenant directeur');
  });

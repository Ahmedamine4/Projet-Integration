...
describe('validation_etudiant.controller', () => {
  ...
  it('validateStudentController retourne 200 si la mise a jour reussit avec rejete', async () => {
    serviceMock.updateValidationStatus.mockResolvedValue({ statut: 'rejete' });
    const req = {
      params: { etudiantId: 'u1', institutionId: 'inst-1' },
      body: { status: 'rejete' },
    };
    ...
    expect(serviceMock.updateValidationStatus).toHaveBeenCalledWith('u1', 'inst-1', 'rejete');
    expect(res.status).toHaveBeenCalledWith(200);
  });
});

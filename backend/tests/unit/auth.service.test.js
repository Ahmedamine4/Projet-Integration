...
describe('auth.service', () => {
  beforeEach(() => {
    ...
    prismaMock.$transaction.mockImplementation(async (callback) =>
      callback({
        utilisateur: prismaMock.utilisateur,
        etudiant: prismaMock.etudiant,
      })
    );
  });
  ...
  it('registerLocalUser cree un utilisateur local avec role etudiant', async () => {
    ...
    expect(prismaMock.etudiant.create).toHaveBeenCalledWith({
      data: {
        etudiant_utilisateur_id: 'u1',
      },
    });
  ...
  it('syncGoogleUser cree un user google et sa ligne etudiant si user inexistant', async () => {
    // Arrange
